  ...
  return (
    <div className="space-y-8 pb-16 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <aside className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6 lg:sticky lg:top-24" id="filter-sidebar">
          
          {/* Sidebar Header */}
          <div className="flex items-center justify-between">
            <h2 className="font-bold text-white text-base tracking-wide uppercase">
              {t.filterTitle[currentLang]}
            </h2>
            <button
              id="clear-filter-btn"
              onClick={handleResetFilters}
              className="text-xs text-slate-400 hover:text-emerald-400 transition-colors font-medium underline underline-offset-2"
            >
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-4 py-2.5 text-slate-200 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
            >
            ...
            <div className="grid grid-cols-2 gap-2" id="filter-category-grid">
              {[
                ...
                <button
                  ...
                  className={`px-3 py-2 text-xs rounded-xl border transition-all font-medium ${
          ...
          <div className="space-y-3 pt-2 border-t border-slate-800/80">
            ...
            <label className="flex items-center space-x-3 cursor-pointer group" id="halal-checkbox-label">
              <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${
                isHalalOnly ? 'bg-emerald-500 border-emerald-500 text-slate-950' : 'border-slate-700 bg-slate-950 group-hover:border-slate-600'
              }`}>
            ...
            <label className="flex items-center space-x-3 cursor-pointer group" id="cdt-checkbox-label">
              <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${
                isCDTOnly ? 'bg-emerald-500 border-emerald-500 text-slate-950' : 'border-slate-700 bg-slate-950 group-hover:border-slate-600'
              }`}>
              ...
              {[
                ...
                <label key={option.id} className="flex items-center space-x-3 cursor-pointer" id={`dress-option-${option.id}`}>
                  <input
                    ...
                    className="w-4 h-4 text-emerald-500 bg-slate-950 border-slate-700 focus:ring-emerald-500 focus:ring-offset-slate-900"
                  />
          ...
          <div className="space-y-3 pt-4 border-t border-slate-800/80 bg-slate-950/40 p-3.5 rounded-xl border border-slate-800/50">
            <div className="flex items-center justify-between">
        ...
        <section className="lg:col-span-3 space-y-6">
          ...
          <div className="flex items-center justify-between bg-slate-900 border border-slate-800 px-5 py-3 rounded-2xl">
            <span className="text-sm font-semibold text-slate-300" id="results-count">
            ...
            {searchQuery && (
              <span className="text-xs bg-slate-800 text-slate-300 px-2.5 py-1 rounded-lg">
                คีย์เวิร์ด: <span className="text-emerald-400 font-semibold">"{searchQuery}"</span>
              </span>
            )}
          </div>
          ...
          {filteredPlaces.length > 0 ? (
              ...
              {filteredPlaces.map((place) => {
                ...
                return (
                  <div
                    ...
                    className="bg-slate-900 border border-slate-800 hover:border-slate-700/80 rounded-2xl overflow-hidden transition-all duration-300 flex flex-col group shadow-lg"
                  >
                    {/* Card Image Wrapper */}
                    <div className="relative h-48 overflow-hidden bg-slate-950">
                      ...
                      {/* Floating badging */}
                      <div className="absolute top-3 left-3 bg-slate-950/85 backdrop-blur-md text-white text-xs px-2.5 py-1 rounded-lg border border-slate-800 font-medium flex items-center gap-1">
                      ...
                      <div 
                        ...
                        className={`absolute top-3 right-3 backdrop-blur-md px-2.5 py-1 rounded-lg text-xs font-bold shadow-md border flex items-center gap-1.5 ${
                          isOpen 
                            ? 'bg-emerald-950/85 text-emerald-400 border-emerald-500/20' 
                            : 'bg-rose-950/85 text-rose-400 border-rose-500/20'
                        }`}
                      >
                    ...
                    <div className="p-5 flex-grow flex flex-col justify-between space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <h3 className="font-bold text-white group-hover:text-emerald-400 transition-colors text-lg">
                      ...
                      <div className="pt-4 border-t border-slate-800/80 flex flex-col gap-3">
                        ...
                        <div className="flex items-center justify-between pt-1">
                          <div className="text-xs text-emerald-400 font-semibold flex items-center gap-1 bg-emerald-500/5 px-2 py-1 rounded-md border border-emerald-500/10">
                          ...
                          <a
                            ...
                            className="bg-slate-800 hover:bg-emerald-500 hover:text-slate-950 font-bold px-3 py-1.5 rounded-xl text-xs transition-all flex items-center gap-1 text-slate-100"
                          >
            ...
            // No Results Screen
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center space-y-4" id="no-results-view">
              ...
              <button
                ...
                onClick={handleResetFilters}
                className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold px-5 py-2 rounded-xl text-xs transition-all shadow-md active:scale-98"
              >
                {t.resetBtn[currentLang]}
              </button>
            </div>
