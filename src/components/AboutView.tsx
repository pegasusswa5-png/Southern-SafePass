...
export default function AboutView({ currentLang }: AboutViewProps) {
  ...
  return (
    <div className="space-y-16 pb-16 animate-in fade-in duration-300">
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-12 relative overflow-hidden" id="about-intro-section">
        ...
        <div className="max-w-3xl mx-auto text-center space-y-6 relative z-10">
          <div className="inline-flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider">
            <GraduationCap className="w-4 h-4" />
            <span>{t.schoolName[currentLang]}</span>
          </div>

          <h2 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">
            {t.teamIntro[currentLang]} — <span className="bg-gradient-to-r from-emerald-400 to-indigo-400 bg-clip-text text-transparent">Pegasus Team</span>
          </h2>
          ...
          <div className="pt-4 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-lg mx-auto" id="brand-values">
            <div className="bg-slate-950 border border-slate-800 p-3 rounded-xl">
              ...
              <p className="text-[10px] font-bold text-white uppercase">Safety Centered</p>
            </div>
            <div className="bg-slate-950 border border-slate-800 p-3 rounded-xl">
              <Heart className="w-5 h-5 text-rose-400 mx-auto mb-1" />
              <p className="text-[10px] font-bold text-white uppercase">Community First</p>
            </div>
            <div className="bg-slate-950 border border-slate-800 p-3 rounded-xl">
              <Sparkles className="w-5 h-5 text-indigo-400 mx-auto mb-1" />
              <p className="text-[10px] font-bold text-white uppercase">User Friendly</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6" id="team-members-grid">
          {teamMembers.map((member, i) => (
            <div
              key={i}
              id={`member-card-${i}`}
              className="bg-slate-900 border border-slate-800/80 rounded-2xl overflow-hidden flex flex-col justify-between hover:border-slate-700 transition-all duration-300 group shadow-md"
            >
              ...
              <div className="relative aspect-square overflow-hidden bg-slate-950">
                ...
                <div className="absolute bottom-3 left-3 flex gap-1.5">
                  <span className="bg-slate-950/80 backdrop-blur-md text-[9px] text-white font-mono border border-slate-800 px-1.5 py-0.5 rounded">
                    PEGASUS
                  </span>
                </div>
              </div>
              ...
              <div className="p-4 flex-grow flex flex-col justify-between space-y-3">
                ...
                <p className="text-[11px] text-slate-500 italic leading-relaxed border-t border-slate-950 pt-2.5">
      ...
      <section className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 md:p-8" id="advisor-section">
        <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8 max-w-4xl mx-auto">
          
          {/* Advisor Image/Icon placeholder */}
          <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center shrink-0 shadow-inner">
